...
/** Platform dependent io register type */
typedef std::uint32_t ioreg_t;

/** Platform dependent io index type */
typedef std::uint32_t ioindex_t;
...
struct register_traits {
  ...
  typedef hw_base::address_type address_holder;
  enum { address_mode, access_mode, endianness, device_bus_width };
};
 ...
 * hardware register.
 */
template<class RegisterTraits = register_traits, class PlatformTraits = platform_traits>
